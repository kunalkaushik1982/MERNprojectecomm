...
exports.resetPassword = catchAsyncErrors(async (req,res,next)=>{
  ...
  //creating token hash
    ...
    if(req.body.password !== req.body.confirmPassword){return next(new ErrorHander("Password doesn't match",400))}
...
exports.updatePassword = catchAsyncErrors(async(req,res,next)=>{
  ...
  if(req.body.newPassword !== req.body.confirmPassword){return next(new ErrorHander("Password doesn't match", 400))}
...
exports.updateUserRole = catchAsyncErrors(async(req,res,next)=>{
  ...
  const user = await User.findById(req.params.id)
  if(!user){return next(new ErrorHander(`User doesn't exist with Id:${req.params.id}`,400))}  
